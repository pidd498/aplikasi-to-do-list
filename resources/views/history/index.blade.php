...
<head>
    ...
    <style>
        *{font-family:'Outfit',sans-serif;transition:background-color 0.3s,color 0.3s,border-color 0.3s;}
        ...
        .tf-input{background:var(--input-bg);border:1px solid var(--border);color:var(--text);border-radius:12px;padding:8px 12px;font-size:14px;outline:none;transition:all 0.2s;font-family:'Outfit',sans-serif;}
        ...
        .filter-tab{padding:8px 16px;border-radius:10px;font-size:14px;font-weight:600;cursor:pointer;text-decoration:none;transition:all 0.2s;color:var(--text3);}
        .filter-tab:hover{color:var(--text);background:var(--hover);}
        .filter-tab.active{background:var(--active-bg);color:var(--blue-light);border:1px solid var(--active-border);}
...
<section class="flex h-screen overflow-hidden">
    ...
    <div class="sidebar w-64 flex flex-col gap-6 p-6 h-screen shrink-0 overflow-y-auto">
        ...
        <div class="flex flex-col gap-1">
            ...
            <a href="{{ route('inbox.index') }}" class="nav-item"><svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4"/></svg>
                Inbox
                @if($notificationCount > 0)
                    <span class="ml-auto gbg text-white text-xs font-bold px-2 py-0.5 rounded-full">{{ $notificationCount }}</span>
                @endif
            </a>
            <a href="{{ route('history.index') }}" class="nav-item active"><svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>History</a>
        </div>
    ...
    <div class="flex-1 flex flex-col overflow-hidden" style="background:var(--bg)">
        <div class="p-6 pb-0">
            <div class="topbar flex justify-between items-center px-6 py-4 fade-up">
                <div>
                    <h1 class="text-xl font-black" style="color:var(--text)">History</h1>
        ...
        <div class="flex-1 overflow-y-auto p-6 flex flex-col gap-5">
            ...
            <!-- Filter bar -->
            <div class="tf-card p-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 fade-up d1">
                <!-- Tabs -->
                <div class="flex items-center gap-2">
                    <a href="{{ route('history.index', ['filter' => 'All', 'tanggal_mulai' => $tanggalMulai, 'tanggal_selesai' => $tanggalSelesai]) }}"
                       class="filter-tab {{ $filter == 'All' ? 'active' : '' }}">All</a>
                    <a href="{{ route('history.index', ['filter' => 'Task', 'tanggal_mulai' => $tanggalMulai, 'tanggal_selesai' => $tanggalSelesai]) }}"
                       class="filter-tab {{ $filter == 'Task' ? 'active' : '' }}">Tasks</a>
                    <a href="{{ route('history.index', ['filter' => 'Todo', 'tanggal_mulai' => $tanggalMulai, 'tanggal_selesai' => $tanggalSelesai]) }}"
                       class="filter-tab {{ $filter == 'Todo' ? 'active' : '' }}">Todos</a>
                </div>

                <!-- Date filter -->
                <form action="{{ route('history.index') }}" method="GET" class="flex items-center gap-2 flex-wrap">
                    <input type="date" name="tanggal_mulai" value="{{ $tanggalMulai }}" class="tf-input text-sm">
                    <span class="text-sm" style="color:var(--text3)">→</span>
                    <input type="date" name="tanggal_selesai" value="{{ $tanggalSelesai }}" class="tf-input text-sm">
                    <input type="hidden" name="filter" value="{{ $filter }}">
                    <button type="submit" class="gbg text-white font-bold px-5 py-2 rounded-xl text-sm hover:opacity-90 transition-opacity">
                        Filter
                    </button>
                </form>
            </div>
            ...
            <div class="tf-card overflow-hidden fade-up d2">
                <div class="px-6 py-4 flex items-center justify-between" style="border-bottom:1px solid var(--border)">
                    <h2 class="font-bold" style="color:var(--text)">
                        {{ $filter == 'All' ? 'All Records' : ($filter == 'Task' ? 'Task History' : 'Todo History') }}
                        <span class="text-sm font-normal ml-1" style="color:var(--text3)">({{ count($tasks) }} records)</span>
                    </h2>
                    @if($tanggalMulai || $tanggalSelesai)
                    <span class="text-xs font-semibold px-3 py-1 rounded-full" style="background:var(--active-bg);color:var(--blue-light);border:1px solid var(--active-border)">
                    ...
                    <table class="tf-table">
                        <thead>
                            <tr>
                                ...
                                <th>Category</th>
                                <th>Created At</th>
                                @if($filter == 'Todo')<th>Date</th>@endif
                                <th>Status</th>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="font-semibold" style="color:var(--text)">{{ $task->task_name }}</td>
                                <td>
                                    <span class="text-xs font-bold px-2.5 py-1 rounded-full" style="background:var(--active-bg);color:var(--blue-light);border:1px solid var(--active-border)">
                                        {{ $task->category ?? 'Task' }}
                                    </span>
                                </td>
                                <td style="color:var(--text2)">{{ \Carbon\Carbon::parse($task->created_at)->format('d M Y, H:i') }}</td>
                                @if($filter == 'Todo')
                                <td style="color:var(--text2)">{{ $task->tanggal ?? '-' }}</td>
                                @endif
                                <td>
                                    @if($task->status == 'Done' || $task->status == '1' || $task->status == true)
                                    ...
                                    @else
                                        <span class="text-xs font-bold px-3 py-1 rounded-full" style="background:rgba(107,114,128,0.15);color:#9ca3af;border:1px solid rgba(107,114,128,0.3)">{{ $task->status ?? 'Pending' }}</span>
                                    @endif
                                </td>
                            </tr>
                            @empty
                            <tr>
                                <td colspan="{{ $filter == 'Todo' ? 6 : 5 }}" class="text-center py-16">
                                    <div class="flex flex-col items-center gap-3">
                                        <div class="w-16 h-16 rounded-2xl flex items-center justify-center" style="background:var(--bg3)">
                                            <svg class="w-8 h-8" style="color:var(--text3)" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                                        </div>
                                        <p class="font-semibold" style="color:var(--text3)">No history found</p>
                                        <p class="text-sm" style="color:var(--text3)">Try changing the filter or date range</p>
                                    </div>
                                </td>
                            </tr>
                            @endforelse
